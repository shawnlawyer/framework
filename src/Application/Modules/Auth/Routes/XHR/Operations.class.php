...
class Operations {
    ...
    public static function login($json = null){
        ...
        if(!isset($error)){
            $dialog_store->step++;
            SessionStore::set($dialog->session_store_key, $dialog_store);
            return (intval($dialog_store->step) == 2) ? \Sequode\Application\Modules\ApplicationConsole\Routes\Routes::js(false) : forward_static_call_array(array($xhr_cards,__FUNCTION__),array());
        }
    }
}